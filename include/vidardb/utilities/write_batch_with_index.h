...
// an entry for Put, Delete entry for write batches.
...
// A WriteBatchWithIndex with a binary searchable index built for all the keys
// inserted.
// In Put(), Delete() the same function of the
...
class WriteBatchWithIndex : public WriteBatchBase {
 public:
  ...
  // regardless).
  Status GetFromBatchAndDB(DB* db, ReadOptions& read_options, const Slice& key,
                           std::string* value);
