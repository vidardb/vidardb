...
TransactionDBImpl::~TransactionDBImpl() {
  ...
  for (const auto& it : expirable_transactions_) {
    delete it.second;
  }
}
...
Transaction* TransactionDBImpl::BeginInternalTransaction(
  ...
  Transaction* txn = BeginTransaction(options, TransactionOptions(), nullptr);
...
//
// Put() and Delete() only lock a single key per call. Write() will sort its
// keys before locking them. This guarantees that TransactionDB write methods
// cannot deadlock with each other (but still could deadlock with a
// Transaction).
...
void TransactionDBImpl::InsertExpirableTransaction(TransactionID tx_id,
  ...
  std::lock_guard<std::mutex> lock(expire_txn_map_mutex_);
  expirable_transactions_.insert({tx_id, tx});
}

void TransactionDBImpl::RemoveExpirableTransaction(TransactionID tx_id) {
  std::lock_guard<std::mutex> lock(expire_txn_map_mutex_);
  expirable_transactions_.erase(tx_id);
}
...
bool TransactionDBImpl::TryStealingExpiredTransactionLocks(
  ...
  auto tx_it = expirable_transactions_.find(tx_id);
  if (tx_it == expirable_transactions_.end()) {
    return true;
  }
...
Status TransactionDB::Open(
  ...
  DB* db;
  Status s =
      DB::Open(db_options_2pc, dbname, column_families_copy, handles, &db);
  ...
  if (s.ok()) {
    ...
    *dbptr = txn_db;
    ...
    auto recovered_txns = dbimpl->recovered_transactions();

    for (const auto& it : recovered_txns) {
      auto recovered_txn = it.second;
      assert(recovered_txn);
      assert(recovered_txn->log_number_);
      assert(recovered_txn->name_.length());

      WriteOptions w_options;
      w_options.sync = true;

      Transaction* txn =
          txn_db->BeginTransaction(w_options, TransactionOptions(), nullptr);
      assert(txn);
      txn->SetLogNumber(recovered_txn->log_number_);

      s = txn->SetName(recovered_txn->name_);
      if (!s.ok()) {
        break;
      }

      s = txn->RebuildFromWriteBatch(recovered_txn->batch_);
      txn->exec_status_ = Transaction::PREPARED;
      if (!s.ok()) {
        break;
      }
    }
