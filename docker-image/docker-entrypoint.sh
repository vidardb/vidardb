...
# setup pg shared preload libraries for foreign data wrapper
pg_setup_shared_preload_libraries() {
	local key="shared_preload_libraries"
	{
		echo
		echo "$key = 'kv_fdw'"
	} >> "$PGDATA/postgresql.conf"

	echo "PostgreSQL print shared preload libraries..."
	cat "$PGDATA/postgresql.conf" | grep "$key"
}
...
docker_temp_server_start() {
	...
	set -- "$@" -c listen_addresses="${PGHOST:-127.0.0.1}" -p "${PGPORT:-5432}"
